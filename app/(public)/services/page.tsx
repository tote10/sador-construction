...
import { 
  ...
  Settings, PenTool, ClipboardCheck, PlayCircle, ShieldAlert 
} from 'lucide-react';
...
export default function ServicesPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      document.title = `Our Services | ${seoSettings.title || 'Sador Construction'}`;
    }
  }, [seoSettings.title]);
  ...
  const deliverySteps = [
    ...
    },
    {
      ...
      subtitle: 'Phase 03',
      desc: 'Heavy grading, bulk soil displacement, trenching for drainage lines, and base preparation works using Sador\'s heavy compaction rollers and machinery assets.',
        ...
        <section className="bg-slate-900 text-white relative py-20 px-6 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto text-center relative z-20 space-y-6">
            ...
            <p className="text-slate-300 font-medium max-w-2xl mx-auto leading-relaxed text-base sm:text-lg">
              We provide full-spectrum Grade-1 general contracting services. From complex civic highways to commercial high-rises, we build for longevity.
            </p>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto space-y-16">
            ...
            <div className="text-center max-w-2xl mx-auto space-y-4">
              ...
              <p className="text-sm text-slate-500 font-semibold">
                Explore our execution methodologies and sub-disciplines. Add or edit these dynamically inside the Admin Panel!
              </p>
            </div>
            ...
            <div className="space-y-12">
              ...
              {services.length === 0 && (
                <div className="text-center py-12 text-slate-500">
                  No services configured. Please log in and define services inside the CMS panel.
                </div>
              )}
            </div>
        ...
        <section className="py-24 px-6 bg-brand-light border-t border-slate-100">
          <div className="max-w-7xl mx-auto space-y-16">
            ...
            <div className="text-center max-w-2xl mx-auto space-y-4">
              ...
              <p className="text-sm text-slate-500 font-semibold">Our structural critical path planning guidelines ensure project deliveries within target schedules.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                ...
                {deliverySteps.map((step, idx) => (
                  <button 
                    ...
                    className={`w-full text-left p-5 rounded-2xl border transition-all duration-300 flex items-center gap-4 ${
                      idx === activeStep 
                        ? 'bg-white border-brand-gold shadow-sm translate-x-2' 
                        : 'bg-transparent border-transparent hover:bg-slate-200/40 text-slate-500'
                    }`}
                  >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                      idx === activeStep ? 'bg-brand-blue text-white' : 'bg-white text-slate-400 border border-slate-100'
                    }`}>
                      {idx + 1}
                    </div>
                    <div>
                      ...
                      <h4 className={`text-sm sm:text-base font-extrabold transition-colors ${
                        idx === activeStep ? 'text-brand-blue' : 'text-slate-700'
                      }`}>{step.title}</h4>
                    </div>
                  </button>
                ))}
              ...
              <div className="lg:col-span-7 bg-white p-8 sm:p-10 rounded-3xl border border-slate-100 shadow-[0_10px_35px_rgba(0,0,0,0.02)] min-h-[300px] flex flex-col justify-between">
                ...
                <div className="pt-8 border-t border-slate-100 flex justify-between items-center">
                  ...
                  <Link 
                    href="/contact" 
                    className="inline-flex items-center gap-1 text-xs font-bold text-brand-gold hover:text-brand-blue uppercase tracking-widest transition-colors"
                  >
                    Consult Engineers
                    <ArrowRight size={14} />
                  </Link>
