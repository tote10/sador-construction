...
export default function BlogClient({ posts = [], seoSettings = {} }: any) {
  useEffect(() => {
    document.title = `Posts | ${seoSettings?.title || 'Sador General Construction'}`;
  }, [seoSettings?.title]);

  const visiblePosts = (posts || []).filter((post: any) => post.published);
      ...
      <main className="flex-grow pt-28">
        <section className="bg-brand-light py-20 px-6 border-b border-slate-100">
          <div className="max-w-4xl mx-auto text-center space-y-5">
            ...
            <p className="text-slate-600 font-medium leading-relaxed max-w-2xl mx-auto">Short company announcements, project highlights, and industry insights curated for clients and partners.</p>
          </div>
        </section>
        ...
        <section className="py-20 px-6 bg-white">
          <div className="max-w-7xl mx-auto">
            {visiblePosts.length === 0 ? (
              <div className="text-center py-16 bg-slate-50 rounded-3xl border border-slate-100 text-slate-500 font-semibold">No published posts yet. The admin can create one from the dashboard.</div>
            ) : (
