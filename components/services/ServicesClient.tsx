...
import { 
  ...
  Settings, PenTool, ClipboardCheck, PlayCircle
} from 'lucide-react';
import Link from 'next/link';

export default function ServicesClient({ services, seoSettings }: { services?: any[]; seoSettings?: any }) {
      ...
      <main className="flex-grow pt-28">
        ...
        <section className="bg-slate-900 text-white relative py-20 px-6 overflow-hidden">
          ...
          <div className="absolute inset-0 z-0">
            <Image src="https://images.unsplash.com/photo-1541888086425-d81bb19240f5?q=80&w=1600" alt="Highway Work" fill priority className="object-cover opacity-30" sizes="100vw" />
          </div>
          <div className="max-w-4xl mx-auto text-center relative z-20 space-y-6">
            ...
            <p className="text-slate-300 font-medium max-w-2xl mx-auto leading-relaxed text-base sm:text-lg">We provide general contracting services shaped by disciplined delivery for buildings, roads, and civil infrastructure.</p>
          </div>
        </section>
        ...
        <section className="py-24 px-6 bg-white">
          <div className="max-w-7xl mx-auto space-y-16">
            <div className="text-center max-w-2xl mx-auto space-y-4">
              ...
              <p className="text-sm text-slate-500 font-semibold">Explore the execution disciplines that support the project delivery model.</p>
            </div>
            ...
            <div className="space-y-12">
              {(services ?? []).map((service: any, idx: number) => {
                ...
                return (
                  <div key={service.id} className={`bg-brand-light p-8 sm:p-12 rounded-3xl border border-slate-100/80 shadow-sm flex flex-col lg:flex-row gap-12 items-center hover:border-brand-gold/25 transition duration-300 ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                    <div className="lg:w-5/12 space-y-6">
                      <div className="w-16 h-16 bg-white border border-slate-150 rounded-2xl flex items-center justify-center shadow-sm">{getServiceIcon(service.icon_name || service.iconName)}</div>
                      ...
                      <p className="text-slate-600 font-medium leading-relaxed text-sm sm:text-base">{service.description}</p>
                    </div>
                    ...
                    <div className="lg:w-7/12 w-full">
                      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-100/60 shadow-inner space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {(service.details ?? []).map((detail: any, dIdx: number) => (
              ...
              {(services ?? []).length === 0 && (
                <div className="text-center py-12 text-slate-500">No services configured yet. Update the service list from the shared content data.</div>
              )}
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {projectHighlights.map((h, i) => (
                <div key={i} className="p-4 rounded-lg border bg-brand-light/60"><h4 className="text-sm font-bold text-brand-blue">{h}</h4></div>
              ))}
            </div>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              <div className="lg:col-span-5 space-y-3">
                {deliverySteps.map((step, idx) => (
                  <button key={step.title} onClick={() => setActiveStep(idx)} className={`w-full text-left p-5 rounded-2xl border transition-all duration-300 flex items-center gap-4 ${idx === activeStep ? 'bg-white border-brand-gold shadow-sm translate-x-2' : 'bg-transparent border-transparent hover:bg-slate-200/40 text-slate-500'}`}>
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${idx === activeStep ? 'bg-brand-blue text-white' : 'bg-white text-slate-400 border border-slate-100'}`}>{idx + 1}</div>
                    <div>
              ...
              <div className="lg:col-span-7 bg-white p-8 sm:p-10 rounded-3xl border border-slate-100 shadow-[0_10px_35px_rgba(0,0,0,0.02)] min-h-[300px] flex flex-col justify-between">
                <div className="space-y-6">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-5">
                    ...
                    <div className="w-10 h-10 rounded-xl bg-brand-light flex items-center justify-center">{deliverySteps[activeStep].icon}</div>
                  </div>
                ...
                <div className="pt-8 border-t border-slate-100 flex justify-between items-center">
                  ...
                  <Link href="/contact" className="inline-flex items-center gap-1 text-xs font-bold text-brand-gold hover:text-brand-blue uppercase tracking-widest transition-colors">Consult Engineers <ArrowRight size={14} /></Link>
                </div>
              </div>
            </div>
          </div>
